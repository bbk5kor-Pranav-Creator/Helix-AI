...
export default function KnowledgeIntelligence() {
  ...
  const exportKnowledgePdf = () => {
    if (!knowledge) return

    const title = sourceInfo?.title || 'Knowledge Intelligence'
    const sourceUrl = sourceInfo?.url || sourceInfo?.filename || 'N/A'
    const sourceType =
      sourceInfo?.source_type === 'website'
        ? 'Website'
        : sourceInfo?.source_type === 'pdf'
          ? 'PDF'
          : 'Source'
    ...
    const relationshipsToMarkdown = (relationships = []) =>
      relationships
        .filter((rel) => rel && (rel.source || rel.relationship || rel.target))
        .map(
          (rel) =>
            `- **${rel.source || 'Unknown'}** → ${
...
${listToMarkdown(knowledge.topics) || '- No topics identified.'}

## Key Facts

${listToMarkdown(knowledge.key_facts) || '- No key facts identified.'}

## Entities

${listToMarkdown(knowledge.entities) || '- No entities identified.'}

## Systems

${listToMarkdown(knowledge.systems) || '- No systems identified.'}

## Tools

${listToMarkdown(knowledge.tools) || '- No tools identified.'}

## Technologies

${listToMarkdown(knowledge.technologies) || '- No technologies identified.'}

## Processes

${listToMarkdown(knowledge.processes) || '- No processes identified.'}

## Risks / Considerations

${listToMarkdown(knowledge.risks) || '- No risks or considerations identified.'}

## Knowledge Relationships

${relationshipsToMarkdown(knowledge.relationships) || '- No relationships identified.'}

## Suggested Questions

${listToMarkdown(knowledge.suggested_questions) || '- No suggested questions.'}
`.trim()

    const formattedAnswer = markdownToPdfHtml(reportMarkdown)
    ...
    doc.write(`
      ...
      <html>
        ...
        <body>
          ...
          <div class="meta">
            <div class="meta-row"><span class="label">Source Type:</span> ${escapeHtml(sourceType)}</div>
