...
export default function UploadPDF() {
  ...
  const analysisModes = [
    {
      ...
      label: 'Summary',
      icon: '✦',
      instruction: 'Give me a concise, well-organized summary of this document. Include only the most important information and avoid unnecessary details.'
    },
    {
      id: 'detailed',
      label: 'Detailed',
      icon: '≡',
      instruction: 'Give me a detailed but well-organized explanation of the important information in this document. Use clear headings and bullet points where useful.'
    },
    {
      id: 'extract',
      label: 'Extract',
      icon: '⌕',
      instruction: 'Extract the specific information that is most relevant from this document. Do not add unrelated information. Clearly state when the requested information is not available.'
    },
    {
      id: 'tables',
      label: 'Tables & Data',
      icon: '▦',
      instruction: 'Identify and summarize the key tables, figures, and numerical data in this document, preserving the relationships between rows and columns.'
    },
    {
      id: 'research',
      label: 'Research',
      icon: '◇',
      instruction: 'Analyze this document as a research source. Extract the key concepts, facts, findings, terminology, and useful points, and organize them clearly without inventing information.'
    },
    {
      id: 'custom',
      label: 'Custom',
  ...
  const handleFileSelect = (file) => {
    ...
    if (file.size > MAX_PDF_MB * 1024 * 1024) {
      setFileError(`The PDF exceeds the ${MAX_PDF_MB} MB upload limit.`)
      return
    }
  ...
  const exportPdf = () => {
    ...
    const sourceFile = result.filename || pdfFile?.name || 'document.pdf'
    const sourceTitle = result.source_title || title
    const analysisInstruction = result.instruction || instruction || 'Document analysis'
    ...
    doc.write(`
      ...
      <html>
        ...
        <body>
          ...
          <div class="meta">
            <div class="meta-row"><span class="label">File:</span> ${escapeHtml(sourceFile)}</div>
      ...
      const documentXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
        <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
          <w:body>
            ...
            <w:p><w:r><w:rPr><w:b/></w:rPr><w:t>File: </w:t></w:r><w:r><w:t>${escapeXml(sourceFile)}</w:t></w:r></w:p>
  ...
  const loadHistory = async () => {
    try {
      ...
      setHistoryError('')
      const response = await axios.get('/api/pdf-history', {
        params: { limit: 100 },
        timeout: 30000
      })
  ...
  const openHistoryItem = async (analysisId) => {
    try {
      ...
      setHistoryLoading(true)
      setHistoryError('')
      const response = await axios.get(`/api/pdf-history/${analysisId}`, { timeout: 30000 })
      ...
      if (
        ...
        if (item.filename) {
          await startConversation(
            historyResult,
            item.filename,
            item.instruction || 'Provide a clear and useful general analysis of this document.'
          )
        }
      }
  ...
  const deleteHistoryItem = async (analysisId) => {
    try {
      ...
      const response = await axios.delete(`/api/pdf-history/${analysisId}`, { timeout: 30000 })
  ...
  const clearAllHistory = async () => {
    ...
    try {
      ...
      setHistoryError('')
      const response = await axios.delete('/api/pdf-history', { timeout: 30000 })
  ...
  const filteredHistory = useMemo(() => {
    ...
    return history.filter(item => {
      ...
      return title.includes(query) || filename.includes(query) || instr.includes(query)
    })
  ...
  const analyzePDF = async () => {
    ...
    if (!pdfFile) {
      setFileError('Please select a PDF file to analyze.')
      return
    }

    const finalInstruction = instruction.trim() || 'Provide a clear and useful general analysis of this document.'
    ...
    try {
      ...
      const data = response.data
      if (!data.success) {
        setError(data.message || 'PDF analysis failed.')
        setLoadingStage(0)
        return
      }
      ...
      if (err.response?.data) {
        ...
        setError('Something went wrong while analyzing the PDF.')
      }
      setLoadingStage(0)
    } finally {
      setLoading(false)
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2 }}>
            <Box sx={{ minWidth: 0, flex: 1 }}>
              ...
              <Typography sx={{ color: '#2dd4bf', fontSize: '11px', mb: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {item.filename}{item.page_count ? ` · ${item.page_count} ${item.page_count === 1 ? 'page' : 'pages'}` : ''}
              </Typography>
    ...
    <Box sx={{
            ...
            <Box sx={{
              ...
              <motion.div variants={fadeInUp}>
                <Box sx={{ mb: 3.5 }}>
                  ...
                  <Typography sx={{ color: '#9aaac0', fontSize: '14px', mt: 1.1, maxWidth: 690, lineHeight: 1.7 }}>
                    Revisit your PDF analyses, continue conversations, and keep your research organized in one place.
                  </Typography>
                </Box>
              </motion.div>
                ...
                <motion.div variants={scaleIn}>
                  <Paper elevation={0} sx={{
                    ...
                    <Typography sx={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.7, maxWidth: 450, mx: 'auto', mb: 3 }}>
                      Analyze a PDF and your results will automatically appear here.
                    </Typography>
                    <Button
                      onClick={resetAnalysis}
              ...
              {!historyLoading && history.length > 0 && filteredHistory.length === 0 && (
                <Paper elevation={0} sx={{ p: 4, borderRadius: 3, textAlign: 'center', backgroundColor: '#0f172a', border: '1px solid #1e293b' }}>
                  ...
                  <Typography sx={{ color: '#94a3b8', fontSize: '12px' }}>Try a different title, filename, or instruction.</Typography>
                </Paper>
              )}
                ...
                <motion.div variants={fadeInUp}>
                  <Box sx={{ maxWidth: 850, mx: 'auto', textAlign: 'center', mb: 5 }}>
                    ...
                    <Typography sx={{ color: '#94a3b8', fontSize: { xs: '15px', sm: '17px' }, maxWidth: 640, mx: 'auto', mt: 2, lineHeight: 1.7 }}>
                      Give Helix AI a PDF document and ask a question. Get a focused, grounded answer from the information that matters — including tables and figures.
                    </Typography>
                  </Box>
                </motion.div>
              )}
                ...
                <motion.div variants={scaleIn}>
                  <Paper elevation={0} sx={{
                    ...
                    <Box
                      ...
                      sx={{
                        display: 'flex', alignItems: 'center', gap: 1.2, px: 1.6, py: pdfFile ? 1.1 : 2.2,
                        borderRadius: 2.5,
                        backgroundColor: isDragging ? '#12233c' : '#0a1628',
                        border: isDragging ? '1.5px dashed #dc2626' : '1px solid #1e3a5f',
                        cursor: loading ? 'default' : 'pointer',
                        transition: 'border-color 0.15s ease, background-color 0.15s ease',
                        '&:hover': { borderColor: loading ? '#1e3a5f' : '#dc2626' }
                      }}
                    >
                      <input
                        ref={fileInputRef}
                      ...
                      />
                      <Box sx={{ color: '#ef4444', display: 'flex' }}><Upload size={18} strokeWidth={2.2} /></Box>
                      {pdfFile ? (
                        <Box sx={{ flex: 1, minWidth: 0 }}>
                          <Typography sx={{ color: '#e2e8f0', fontSize: '14px', fontWeight: 650, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {pdfFile.name}
                          </Typography>
                          <Typography sx={{ color: '#64748b', fontSize: '11px', mt: 0.2 }}>
                            {formatFileSize(pdfFile.size)}
                          </Typography>
                        </Box>
                      ) : (
                        <Typography sx={{ color: '#64748b', fontSize: '15px', flex: 1 }}>
                    ...
                    {fileError && <Typography sx={{ color: '#f87171', fontSize: '12px', mt: 1, ml: 1 }}>{fileError}</Typography>}
                    ...
                    <Box sx={{
                      ...
                      <TextField
                        ...
                        placeholder={analysisMode === 'custom' ? 'What would you like to know about this document? You can write your own instruction.' : 'Edit this instruction if you want to customize the analysis...'}
                        value={instruction}
                    ...
                    <Box sx={{
                      ...
                      <Button
                        ...
                        startIcon={loading ? null : <Upload size={17} strokeWidth={2.3} />}
              ...
              {loading && (
                <motion.div variants={fadeInUp}>
                  <Paper elevation={0} sx={{
                    ...
                    <LoadingStage number={1} label="Reading PDF content" />
                  ...
                  <Box sx={{ maxWidth: 900, mx: 'auto', mt: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 2, mb: 2.5 }}>
                      <Box>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.7 }}>
                          <Box sx={{ color: '#ef4444', display: 'flex' }}><FileText size={14} strokeWidth={2.3} /></Box>
                          <Typography sx={{ color: '#64748b', fontSize: '12px' }}>
                            {result.filename}{result.page_count ? ` · ${result.page_count} ${result.page_count === 1 ? 'page' : 'pages'}` : ''}
                          </Typography>
                        </Box>
                      </Box>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.1 }}>
                          ...
                          <Box>
                            ...
                            <Typography sx={{ color: '#64748b', fontSize: '9.5px', mt: 0.2 }}>
                              Continue exploring the same document
                            </Typography>
                          </Box>
                        </Box>
                    ...
                    <Box sx={{ mt: 2.5, px: 1, display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', flexDirection: { xs: 'column', sm: 'row' }, gap: 1.5 }}>
                      <Box>
                        ...
                        <Typography sx={{ color: '#ef4444', fontSize: '12.5px', wordBreak: 'break-all' }}>{result.filename || result.source_url}</Typography>
                      </Box>
                      {result.page_count ? (
                        <Box sx={{
                          display: 'inline-flex', alignItems: 'center', gap: 0.6, px: 1.2, py: 0.6, borderRadius: 5,
                          backgroundColor: 'rgba(148,163,184,0.12)', border: '1px solid #334155',
                          color: '#94a3b8', fontSize: '11.5px', fontWeight: 650
                        }}>
      ...
      <Box sx={{ maxWidth: 900, mx: 'auto', textAlign: 'center', mt: result ? 2 : 8, pb: 3 }}>
        <Typography sx={{ color: '#475569', fontSize: '10.5px', letterSpacing: '0.3px' }}>
          Helix AI · Intelligent PDF analysis
        </Typography>
      </Box>
    </Box>
  )
